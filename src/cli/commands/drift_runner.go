...
func RunDriftDetection(ctx context.Context, projectRoot string, opts DriftOptions) error {
	...
	fmt.Println("Running drift detection tests...")
	fmt.Println()
	...
	for _, specPath := range specs {
		...
		// Print detailed drift detection results
		printDriftResult(rep)
	}

	// Print summary
	fmt.Println()
	printDriftSummary(index)
...
// printDriftResult prints detailed information about a drift detection result
func printDriftResult(rep DriftRunReport) {
	// Print component header
	fmt.Printf("📋 Drift Detection: %s\n", rep.Component)

	// Print overall status
	if rep.Failed == 0 {
		fmt.Printf("   ✅ Status: ALL TESTS PASSED\n")
	} else {
		fmt.Printf("   ❌ Status: %d/%d TESTS FAILED\n", rep.Failed, rep.Total)
	}

	// Print test results
	fmt.Printf("   📊 Results: %d passed, %d failed, %d total\n", rep.Passed, rep.Failed, rep.Total)

	// Print spec file location
	if rep.SpecPath != "" {
		fmt.Printf("   📄 Spec: %s\n", rep.SpecPath)
	}

	// Print detailed test results for failed tests
	if rep.Failed > 0 {
...
// printDriftSummary prints a summary of all drift detection results
func printDriftSummary(reports []DriftRunReport) {
	...
	for _, rep := range reports {
		totalTests += rep.Total
		totalPassed += rep.Passed
		totalFailed += rep.Failed
	}

	fmt.Printf("📊 Drift Detection Summary:\n")
	...
	if totalFailed == 0 {
		fmt.Printf("   🎉 Overall Status: ALL DRIFT TESTS PASSED\n")
	} else {
		fmt.Printf("   💥 Overall Status: SOME DRIFT TESTS FAILED\n")
		fmt.Printf("\n💡 Tip: Check the detailed JSON reports in tests/reports/ for more information\n")
	}
}
