...
func RunGoTests(ctx context.Context, projectRoot string, opts TestRunOptions) error {
    ...
    cfg, _ := loadTestConfig(projectRoot)
    ...
    for _, s := range suites {
        ...
        if opts.Coverage && res.CoveragePct > 0 {
            fmt.Printf("Go %s: %s (coverage=%.1f%% threshold=%d)\n", s, status, res.CoveragePct, res.Threshold)
        } else {
            fmt.Printf("Go %s: %s\n", s, status)
        }
        if !res.Passed {
...
func determineSuites(cfg testConfig, opts TestRunOptions) []string {
    ...
    if opts.UseAll || (!opts.RunUnit && !opts.RunInt && !opts.RunE2E) {
        ...
        if cfg.TestSuites["unit"].Enabled { suites = append(suites, "unit") }
        if cfg.TestSuites["integration"].Enabled { suites = append(suites, "integration") }
        if cfg.TestSuites["e2e"].Enabled { suites = append(suites, "e2e") }
        return suites
    }
    if opts.RunUnit { suites = append(suites, "unit") }
    if opts.RunInt { suites = append(suites, "integration") }
    if opts.RunE2E { suites = append(suites, "e2e") }
    return suites
}
...
func parseFloatFallback(s string) float64 {
    ...
    var f float64
    if _, err := fmt.Sscanf(s, "%f", &f); err == nil { return f }
    return 0
}
...
func writeGoJUnit(path string, rep GoTestsReport) error {
    ...
    for _, r := range rep.Results { if !r.Passed { failures++ } }
    b := &strings.Builder{}
