...
type item struct{ id, content string; score float64 }
...
func (s *sqliteVectorStore) IngestDocuments(ctx context.Context, documents []string) (string, error) {
    ...
    for i, doc := range documents {
        ...
        if _, err := w.Write(by); err != nil { return "", err }
...
func (s *sqliteVectorStore) Search(ctx context.Context, query string, topK int) ([]SearchResult, error) {
    ...
    for scan.Scan() {
        var rec vecRecord
        if err := json.Unmarshal([]byte(scan.Text()), &rec); err != nil { continue }
...
func selectTopK(items []item, k int) {
    ...
    for i := 0; i < k; i++ {
        maxIdx := i
        for j := i + 1; j < n; j++ {
            if items[j].score > items[maxIdx].score { maxIdx = j }
        }
...
func float64sToBytes(v []float64) []byte {
    ...
    for i, f := range v {
        ...
        for j := 0; j < 8; j++ { b[i*8+j] = byte(u >> (8 * j)) }
    }
    return b
}

func bytesToFloat64s(b []byte) []float64 {
    if len(b)%8 != 0 { return nil }
    n := len(b) / 8
    v := make([]float64, n)
    for i := 0; i < n; i++ {
        var u uint64
        for j := 0; j < 8; j++ { u |= uint64(b[i*8+j]) << (8 * j) }
