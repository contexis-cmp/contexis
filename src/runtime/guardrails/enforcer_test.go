...
func TestValidateCapabilities(t *testing.T) {
    ...
    if err := ValidateCapabilities(ctx, "search"); err != nil { t.Fatalf("unexpected: %v", err) }
    if err := ValidateCapabilities(ctx, "write"); err == nil { t.Fatalf("expected error for disallowed capability") }
}
...
func TestEnforceGuardrails(t *testing.T) {
    ...
    if out == "" { t.Fatalf("expected content") }
}
